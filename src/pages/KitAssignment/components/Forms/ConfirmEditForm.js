...
function ConfirmEditForm({
  ...
  const { data, isLoading } = useQuery(
    [KitService.classifications.id],
    KitService.classifications.get
  )
  ...
  const onUpload = async file =>
    KitService.uploadFile.post(file).then(res => {
  ...
  const onSubmit = formData => {
    ...
    if (personId) {
      ...
      handleError({
        error: { message: 'Debe seleccionar un autorizador del autocompletado' }
      })
    }
  }
  return (
        ...
        <div className="content">
          ...
          <div className="simple-row">
            ...
            <FormInput
              control={control}
              name="reference"
              rules={{ required: 'Referencia no ingresada' }}
            />
          </div>
          <div className="simple-row">
            ...
            <FormTextArea
              control={control}
              name="comment"
              rules={{ required: 'Comentario no ingresado' }}
            />
          </div>
          <Accordion className="accordion">
            <AccordionTab header="Montos">
              <div className="simple-row">
                <label htmlFor="kitAmount">Equipo:</label>
                <FormInputNumber
                  control={control}
                  name="kitAmount"
                  mode="decimal"
                  ...
                  rules={{
                    max: {
                      value: 99999,
                      message: 'El monto no debe superar las 99999 cifras'
                    }
                  }}
                />
              </div>
              <div className="simple-row">
                <label htmlFor="penaltyAmount">Penalidad:</label>
                <FormInputNumber
                  control={control}
                  name="penaltyAmount"
                  mode="decimal"
              ...
              <div className="simple-row">
                <label htmlFor="basicIncomeAmount">Renta básica:</label>
                <FormInputNumber
                  control={control}
                  name="basicIncomeAmount"
                  mode="decimal"
                  maxFractionDigits={3}
                  rules={{
                    max: {
              ...
              <div className="simple-row">
                <label htmlFor="ssaaAmount">SSAA:</label>
                <FormInputNumber
                  control={control}
                  name="ssaaAmount"
                  mode="decimal"
              ...
              <div className="simple-row">
                <label htmlFor="othersAmount">Otros:</label>
                <FormInputNumber
                  control={control}
                  name="othersAmount"
                  mode="decimal"
