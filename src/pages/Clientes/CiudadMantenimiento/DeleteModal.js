...
export default function DeleteModal({ onClose, isVisible, service, rowData, postDelete }) {
      ...
      rowData.id,
      {
        onSuccess: () => {
          if (isMounted.current) {
            postDelete()
            toast.success(`Ciudad "${rowData?.description ?? ''}" eliminada con éxito`)
          }
        },
        onError: err => {
          if (isMounted.current) {
  ...
  return (
    <Dialog visible={!!isVisible} onHide={onClose} header="Eliminar ciudad" modal footer={
      <>
      ...
      <p>¿Está seguro que desea eliminar la ciudad <b>{rowData?.description}</b>?</p>
    </Dialog>
  )
}
