...
function ResponsibleEditModal({ onClose, isVisible, defaultFields, updateAssignment }) {
  ...
  const registerData = async formData => {
    ...
    const editionData = {
      ...
      brand: defaultFields.brand.value,
      ...
      operatingSystem: defaultFields?.operatingSystem?.value || null,
      processor: defaultFields?.processor?.value || null,
      inches: defaultFields?.inches?.value || null,
      ip: defaultFields?.ip || '',
      mac: defaultFields?.mac || '',
      domain: defaultFields?.domain?.value || null,
      memory: defaultFields?.memory?.value || null,
      disk: defaultFields?.diskDrive?.value || null
    }
    setIsMutating(true)
    try {
