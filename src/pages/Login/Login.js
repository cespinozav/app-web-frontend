...
import SedeSelectorModal from './SedeSelectorModal'
...
const Login = () => {
  ...
  const onLogin = async formData => {
    ...
    try {
      ...
      // Consultar sedes asociadas
      const res = await (await import('services/SedeCliente')).default.getSedesPorPersona()
      const sedesData = await res.json()
      const sedesArray = Array.isArray(sedesData.result) ? sedesData.result : []
      if (sedesArray.length === 1) {
        // Guardar y continuar directo
        const sede = sedesArray[0]
        const sedeInfo = {
          id_sede: sede.id_sede,
          nombre_sede: sede.nombre_sede,
          id_cliente: sede.id_cliente,
          nombre_cliente: sede.nombre_cliente,
          rol: sede.rol
        }
        setUserinfo({ ...data, sede: sedeInfo })
        localStorage.setItem('selectedSede', JSON.stringify(sedeInfo))
        setPendingUser(null)
        setShowSedeModal(false)
        navigate('/')
      } else if (sedesArray.length > 1) {
        setPendingUser(data)
        setShowSedeModal(true)
      } else {
        toast.error('No tienes sedes asociadas. Contacta al administrador.')
      }
    } catch (e) {
      ...
      <div className="container">
        ...
        <SedeSelectorModal
          ...
          onSelect={sede => {
            ...
            setShowSedeModal(false)
            // Guardar toda la info relevante
            const sedeInfo = {
              id_sede: sede.id_sede,
              nombre_sede: sede.nombre_sede,
              id_cliente: sede.id_cliente,
            ...
            setUserinfo({ ...pendingUser, sede: sedeInfo })
            localStorage.setItem('selectedSede', JSON.stringify(sedeInfo))
            setPendingUser(null)
            navigate('/')
          }}
